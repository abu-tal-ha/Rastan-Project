...
async function appendFoodItemsToHtml(link, callback = () => {}) {
    ...
    try {
        ...
        // data.forEach(food => {
        // document.getElementById('food_gallery').innerHTML += foodCart()
        // })
        ...
        // } else {
        //     let finalOutput = data.data.map(food => {
        //         document.getElementById('food_gallery').innerHTML += foodCart(food);
        //         });
        // }
...
function appendFoodItemsToHtmlTopPopup() {
    let cartHtml = (food) => {
        return ` <tr>
                    <td>
                    ...
                    <td>
                        <div class="quantity-area d-flex align-items-center">
                            <span class="quantity d-block">${food.quantity}</span>
...
function cartItemCount() {
    let cartCountElements = document.querySelectorAll('.cart_item-count ')
...
function addToCartHandler() {
    ...
    Array.from(addToCartBtns).forEach(btn => {
        btn.addEventListener('click', function(event) {
            ...
            cartItemCount();
            appendFoodItemsToHtmlTopPopup();
            changButtonDesign(id);
            totalCount();
        })
    })
...
function totalCount() {
    ...
    let count = activeCartItems.reduce((total, Number) => {
